...
describe('BrainRegiaPanel', () => {
  it('renders authoritative null metrics as insufficient data and exposes pending suggest actions', () => {
    render(<BrainRegiaPanel status="LIVE" snapshot={{ ...snapshot, regiaProposals: [{ id: 'p1', controlMode: 'SUGGEST', status: 'PENDING', commandType: 'CHECK_TABLE', payload: { commandType: 'CHECK_TABLE', scope: { type: 'SESSION' } } }] }} error={null} onApprove={vi.fn()} onReject={vi.fn()} />)
