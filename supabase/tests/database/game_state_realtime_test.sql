...
values
  ...
  ('00000000-0000-0000-0000-000000000075', 'authenticated', 'authenticated', 'realtime-player-2@example.test', true),
  ('00000000-0000-0000-0000-000000000076', 'authenticated', 'authenticated', 'realtime-player-3@example.test', true);
...
values
 ('20000000-0000-0000-0000-000000000071', 'a0000000-0000-0000-0000-000000000050', '00000000-0000-0000-0000-000000000073', 'Realtime Player'),
 ('20000000-0000-0000-0000-000000000075', 'a0000000-0000-0000-0000-000000000050', '00000000-0000-0000-0000-000000000075', 'Realtime Player 2'),
 ('20000000-0000-0000-0000-000000000076', 'a0000000-0000-0000-0000-000000000050', '00000000-0000-0000-0000-000000000076', 'Realtime Player 3');
...
select is((select count(*) from realtime.messages where topic = 'game:a0000000-0000-0000-0000-000000000050' and event = 'game_state_changed'), 4::bigint, 'lifecycle change emits once');
...
insert into public.game_role_assignments (player_id, game_id, role) values
 ('20000000-0000-0000-0000-000000000071','a0000000-0000-0000-0000-000000000050','liar'),
 ('20000000-0000-0000-0000-000000000075','a0000000-0000-0000-0000-000000000050','accomplice'),
 ('20000000-0000-0000-0000-000000000076','a0000000-0000-0000-0000-000000000050','scapegoat');
set local role authenticated;
select set_config('request.jwt.claim.sub', '00000000-0000-0000-0000-000000000071', true);
select set_config('request.jwt.claims', '{"sub":"00000000-0000-0000-0000-000000000071","role":"authenticated","is_anonymous":false}', true);
select * from public.transition_game_narrative_phase('TEST01', 'lobby', 'role_reveal', '50000000-0000-0000-0000-000000000072');
reset role;
select is((select count(*) from realtime.messages where topic = 'game:a0000000-0000-0000-0000-000000000050' and event = 'game_state_changed'), 5::bigint, 'narrative phase change emits once');
update public.games set updated_at = now() where id = 'a0000000-0000-0000-0000-000000000050';
select is((select count(*) from realtime.messages where topic = 'game:a0000000-0000-0000-0000-000000000050' and event = 'game_state_changed'), 5::bigint, 'unrelated update emits nothing');
