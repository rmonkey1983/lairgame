-- ============================================================
-- LIAR SYSTEM - SCHEMA LIVE APP (SUPABASE)
-- Canonico per src/pages/AdminDashboard.tsx e PlayerTerminal.tsx.
-- Idempotente: ripara colonne/policy mancanti senza droppare dati.
-- ============================================================
...
ALTER TABLE games
    ...
    ADD COLUMN IF NOT EXISTS current_liar TEXT,
...
ALTER TABLE games
    ...
    CHECK (status IN ('waiting', 'started', 'running', 'finished'));

UPDATE games
SET scenario_id = gen_random_uuid()
WHERE scenario_id IS NULL;

ALTER TABLE games
    ALTER COLUMN scenario_id SET DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS players (
    ...
    is_accomplice BOOLEAN NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
ALTER TABLE players
    ...
    ADD COLUMN IF NOT EXISTS is_liar BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS is_accomplice BOOLEAN NOT NULL DEFAULT FALSE,
    ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ NOT NULL DEFAULT NOW();

-- Compatibilita con una vecchia bozza di schema che usava name/seat_number.
DO $$
BEGIN
    IF EXISTS (
...
ALTER TABLE players
    ...
    ALTER COLUMN posto_tavola SET NOT NULL;

ALTER TABLE players
    DROP CONSTRAINT IF EXISTS unique_game_seat,
    DROP CONSTRAINT IF EXISTS unique_game_player_name;

ALTER TABLE players
    ADD CONSTRAINT unique_game_player_name UNIQUE (game_id, nickname);

-- ============================================================
-- 3. RISPOSTE / VOTI LIVE
-- ============================================================
...
-- ============================================================
-- 4. EVENTI PROGRAMMATI E MOTORE NO-CODE
-- ============================================================
...
-- ============================================================
-- 7. RLS
-- Nota: l'app attuale non ha login staff. Queste policy abilitano
-- il flusso live con anon key. Prima della produzione pubblica va
-- aggiunta auth admin vera e ristrette le policy di games/events_engine.
-- ============================================================
...
DROP POLICY IF EXISTS "events_engine_select" ON events_engine;
DROP POLICY IF EXISTS "events_engine_insert" ON events_engine;
DROP POLICY IF EXISTS "events_engine_delete" ON events_engine;
...
DROP POLICY IF EXISTS "game_logs_select" ON game_logs;
DROP POLICY IF EXISTS "game_logs_insert" ON game_logs;
